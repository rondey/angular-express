...
export class DateFilterPipe implements PipeTransform {
    ...
    transform(items: any[], field: string, parameters: Parameters): any[] {
        ...
        return items.filter(it => {
            // Converto da stringa a data le due date di input
            const dateField = new Date(it[field]);
            const dateValue = new Date(value);

            // Imposto l'ora della data
            ...
            if (operator.substring(0, 2) === 'gt') {
                ...
                // non decresenza.
                // result assume il valore del confronto fra dateField
                // e dateValue
                result = dateField > dateValue;
            ...
            if (operator.charAt(2) === 'e' || operator.charAt(0) === 'e') {
                ...
                // Entrambe le date vengono convertite in ISO per assicurarsi
                // che possano essere confrontate correttamente
                result = result || dateField.toISOString() === dateValue.toISOString();
            }
            return result;
        });
    }
